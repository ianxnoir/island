...
//   // roomList.innerHTML = `${userRooms.currentUserRooms}`
//   // let roomName = `${userRooms.currentUserRooms}`
//   // document.getElementById('.roomList01').innerHTML = JSON.stringify(roomName)

// }
// getUserRooms()

// ---- Search User ---- (Connected with [homepageRoutes.ts])

const searchBar_user = document.querySelector('.searchBar_user')

searchBar_user.addEventListener("change", async (evt) => {
  const res = await fetch(`/path/${evt.target.value}`)
  const searchUser_result = await res.json()
  console.log(searchUser_result)   //get the user Result as a json
  app.searchUser_result = searchUser_result; // for Vue
})

// ---- Search island/Room ---- (Connected with [homepageRoutes.ts])

const searchBar_room = document.querySelector('.searchBar_island')

searchBar_room.addEventListener("change", async (evt) => {
  const res = await fetch(`/path01/${evt.target.value}`)
  const searchRoom_result = await res.json()
  console.log(searchRoom_result)   //get the user Result as a json
  app.searchRoom_result = searchRoom_result; // for Vue
})
...
const app = new Vue({
  ...
  <div class="card">
    <div class="additional">
      ...
      <div class="more-info">
        <h1>{{searchUser_result.nickname}}</h1>
        <div class="coords">
          <span>Bio</span>
          <span>Aim High & Dream Big{{searchUser_result.bio}}</span>
        </div>
        <div class="coords">
          <span>Email</span>
          <span>{{searchUser_result.email}}</span>
        </div>
        <div class="coords">
          <span>Joined</span>
          <span>{{searchUser_result.joined_at}}</span>
        </div>
        <div class="stats">
          <div>
    ...
    <div class="general">
      <h1>{{searchUser_result.nickname}} </h1>
      <p>{{searchUser_result.bio}}</p>
      <span class="more"> Hover to Know this Islander more</span>
    </div>
  </div>

  <div class="card green" v-for="room in searchRoom_result">
    <div class="additional">
      <div class="user-card">
        <div class="level center">
          Island
        ...
        <svg width="110" height="110" viewBox="0 0 250 250" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="title desc" class="center">
          <title id="title">{{room.name}}</title>
          <desc id="desc">Cartoon of a Caucasian woman smiling, and wearing black glasses and a purple shirt with white collar drawn by Alvaro Montoro.</desc>
          <style>
            .skin {
              fill: #eab38f;
    ...
    <div class="general">
      ...
      <span class="more"> Hover to Know this Island more</span>
    </div>
  </div>
